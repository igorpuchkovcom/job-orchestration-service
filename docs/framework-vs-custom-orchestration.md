# Framework vs Custom Orchestration (Interview Follow-up)
...
"I used custom orchestration here because this repo demonstrates a deterministic, product-specific control plane: API contracts, lifecycle FSM, durable jobs/steps/events, audit events, and idempotent start semantics. For that bounded scope, adding LangGraph first would have been premature and mostly ceremonial. The right boundary is to keep the control plane custom and make the execution/runtime layer pluggable. As workflows become branching, long-running, async, or agentic, LangGraph and workflow engines like Celery or Temporal become strong fits."
